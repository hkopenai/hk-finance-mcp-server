...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"live_api_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
...
try:
    from hkopenai.hk_finance_mcp_server.tool_business_reg import (
...
def test_business_reg():
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched Business Registration data. Sample: {data[:200]}..."
        )
        assert True
...
def test_coin_cart():
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched Coin Cart data. Sample: {str(data)[:200]}..."
        )
        assert True
    except Exception as e:
...
def test_neg_resident_mortgage():
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched Negative Equity Mortgage data. Sample: {data[:200]}..."
        )
        assert True
    except Exception as e:
