...
class TestApp(unittest.TestCase):
    ...
    def test_create_mcp_server(self, mock_tool_fraudulent_bank_scams, mock_tool_bank_branch_locator, mock_tool_stamp_duty_statistics, mock_tool_atm_locator, mock_tool_hibor_daily, mock_tool_hkma_tender, mock_tool_coin_cart, mock_tool_credit_card, mock_tool_neg_resident_mortgage, mock_tool_business_reg, mock_fastmcp):
        # Setup mocks
        mock_server = Mock()
        ...
        decorated_funcs['get_business_stats'](start_year=2020, start_month=1, end_year=2020, end_month=12)
        mock_tool_business_reg.get_business_stats.assert_called_once_with(2020, 1, 2020, 12)

        decorated_funcs['get_neg_equity_stats'](start_year=2020, start_month=1, end_year=2020, end_month=12)
        mock_tool_neg_resident_mortgage.get_neg_equity_stats.assert_called_once_with(2020, 1, 2020, 12)

        decorated_funcs['get_credit_card_stats'](start_year=2020, start_month=1, end_year=2020, end_month=12)
        mock_tool_credit_card.get_credit_card_stats.assert_called_once_with(2020, 1, 2020, 12)
        ...
        mock_tool_hkma_tender.get_tender_invitations.assert_called_once_with(lang="en", segment="tender", pagesize=None, offset=None, from_date=None, to_date=None)

        decorated_funcs['get_hibor_daily_stats'](start_date="2020-01-01", end_date="2020-01-31")
        mock_tool_hibor_daily.get_hibor_stats.assert_called_once_with("2020-01-01", "2020-01-31")

        decorated_funcs['get_atm_locations'](district="Central")
        mock_tool_atm_locator.get_atm_locations.assert_called_once_with("Central", None, 100, 0)

        decorated_funcs['get_stamp_duty_statistics'](start_period="202001", end_period="202012")
        mock_tool_stamp_duty_statistics.get_stamp_duty_statistics.assert_called_once_with("202001", "202012")

        decorated_funcs['get_bank_branch_locations'](district="Central", lang="en")
        mock_tool_bank_branch_locator.get_bank_branch_locations.assert_called_once_with("Central", None, "en", 100, 0)

        decorated_funcs['get_fraudulent_bank_scams'](lang="en")
        mock_tool_fraudulent_bank_scams.get_fraudulent_bank_scams.assert_called_once_with("en")
