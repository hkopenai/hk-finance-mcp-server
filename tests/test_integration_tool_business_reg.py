...
class TestBusinessReturnsIntegration(unittest.TestCase):
    ...
    def test_fetch_business_returns_data_live(self):
        ...
        try:
            ...
            self.assertIn('year_month', result[0], "Expected 'year_month' key in result")
            self.assertIn('active_business', result[0], "Expected 'active_business' key in result")
            self.assertIn('new_registered_business', result[0], "Expected 'new_registered_business' key in result")
    ...
    def test_fetch_business_returns_data_with_filters_live(self):
        ...
        try:
            ...
            result = fetch_business_returns_data(start_year=2023, start_month=1, end_year=2023, end_month=12)
            self.assertIsInstance(result, list)
            self.assertGreater(len(result), 0, "Expected non-empty result from live API with filters")
            for entry in result:
