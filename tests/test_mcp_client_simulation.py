...
class TestMCPClientSimulation(unittest.TestCase):
    ...
    def test_get_business_stats_tool(self):
        ...
        asyncio.run(
            self._call_tool_and_assert(
                "get_business_stats", {"start_year": 2023, "end_year": 2023}
            )
        )
    ...
    def test_get_neg_equity_stats_tool(self):
        ...
        asyncio.run(
            self._call_tool_and_assert(
                "get_neg_equity_stats", {"start_year": 2023, "end_year": 2023}
            )
        )
    ...
    def test_get_credit_card_stats_tool(self):
        ...
        asyncio.run(
            self._call_tool_and_assert(
                "get_credit_card_stats", {"start_year": 2023, "end_year": 2023}
            )
        )
    ...
    def test_get_hkma_tender_invitations_tool(self):
        ...
        asyncio.run(
            self._call_tool_and_assert(
                "get_hkma_tender_invitations",
                {"lang": "en", "segment": "tender", "pagesize": 1},
            )
        )
    ...
    def test_get_stamp_duty_statistics_tool(self):
        ...
        asyncio.run(
            self._call_tool_and_assert(
                "get_stamp_duty_statistics",
                {"start_period": "202301", "end_period": current_year_month},
            )
        )
    ...
    def test_get_bank_branch_locations_tool(self):
        ...
        asyncio.run(
            self._call_tool_and_assert("get_bank_branch_locations", {"pagesize": 1})
        )

    def test_get_fraudulent_bank_scams_tool(self):
        logger.debug("Testing 'get_fraudulent_bank_scams' tool...")
        asyncio.run(
            self._call_tool_and_assert("get_fraudulent_bank_scams", {"lang": "en"})
        )
